  ...
  class MenuCard {
    ...
    render() {
      ...
      element.innerHTML = `
            ...
            `;
      /*Намеренно поместили div .menu__item внутрь созданного div
      Теперь нам нужно получить родтеля, в который будем помещать div .menu__item
      Создаем в классе MenuCard аргумент parentSelector
      */
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
//Для слайдера используется прием деструктуризации (это использование {})
function slider(_ref) {
  ...
  function dotsOpacity() {
    dots.forEach(dot => dot.style.opacity = '.6');
    dots[slideIndex - 1].style.opacity = 1;
  }
  function numWithZero() {
    if (slides.length < 10) {
      current.textContent = `0${slideIndex}`;
    } else {
      current.textContent = slideIndex;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2023-08-30');
  ...
  //Для слайдера используется прием деструктуризации (это использование {}), поэтому здесь мы описываем слайдер
});
})();
